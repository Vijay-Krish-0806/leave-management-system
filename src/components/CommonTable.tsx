...
* @returns {JSX.Element}
*/
const Table = <T extends object>({ columns, data, caption, isLoading = false, isError = false, errorMessage = "Error loading data", }: TableProps<T>) => {
    if (isLoading) {
    ...
    return (<div className="table-container">
      <table className="custom-table">
        ...
        <tbody className="table-body">
          {data && data.length > 0 ? (data.map((item, index) => (<tr key={(item as {
            id?: string | number;
        }).id ?? index}>
                <td>{index + 1}</td>
                {columns.map((column) => (<td key={column.header}>
